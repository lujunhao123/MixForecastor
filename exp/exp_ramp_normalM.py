...
class Exp_Ramp_normalM(Exp_Basic):
    def __init__(self, args):
        super(Exp_Ramp_normalM, self).__init__(args)
    ...
    def vali(self, vali_data, vali_loader, task_dis_criterion,task_con_criterion):
        ...
        with torch.no_grad():
            for i, (batch_x_dis, batch_x_con, batch_y_dis, batch_y_con, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                x_dis_forecast, x_con_forecast = self.model(batch_x_dis, batch_x_con, batch_x_mark, dec_inp_dis, dec_inp_con, batch_y_mark)
                ...
                loss = 0.5 * dis_fore_loss + 0.5 * con_fore_loss
