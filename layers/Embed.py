...
class DataEmbedding_GNN(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', num_nodes=7 ,dropout=0.1):
        super(DataEmbedding_GNN, self).__init__()
        ...
        self.hour_embed = nn.Parameter(torch.empty(24, d_model))
        nn.init.xavier_uniform_(self.hour_embed)
        self.day_embed = nn.Parameter(torch.empty(7, d_model))
    ...
    def forward(self, x, x_mark):
        ...
        if x_mark is not None:
            ...
            x = self.value_embedding(x) + node_emb  + day_embed + hour_embed
        else:
